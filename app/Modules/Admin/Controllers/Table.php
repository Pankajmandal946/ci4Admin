...
class Table extends Controller
{
    ...
    public function table($action = false, $record_id = 0)
    {
        ...
        switch ($action) {
            case 'new':
                ...
                break;
            case 'edit':
                if ($record = $this->userModel->editRecord($record_id)) {
                    $data['edit'] = $record;
                }

                if ($record && $post = $this->request->getPost()) {
                    $this->userModel->editRecord($record_id, $post);
                }
                $data['message'] = $this->session->getFlashData('message');
                echo view('admin/tables-new', $data);
                break;
            case 'delete':
                ...
                if ($record && $post = $this->request->getPost() && $result = $this->userModel->deleteRecord($record_id)) {
